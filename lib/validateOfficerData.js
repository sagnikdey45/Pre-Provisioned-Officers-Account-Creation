...
export const validateAdminData = (data = []) => {
  ...
  data.forEach((row, index) => {
    ...
    const processedAdmin = {
      ...
      city: row.city.trim(),
      district: row.district.trim(),
      password: generatePassword(row.fullName, row.dob)
    }

    // Check duplicate emails
    const emailExists = validAdmins.some(a => a.email === processedAdmin.email)
    if (emailExists) {
      warnings.push(`Row ${rowNumber}: Duplicate email "${processedAdmin.email}" - skipping`)
      return
    }

    validAdmins.push(processedAdmin)
  })

  console.log(`\nValid Admins: ${validAdmins.length}`)
